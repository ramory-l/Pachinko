...
//  Project11
//
//  Created by Mikhail Strizhenov on 17.04.2020.
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        ballsLimitLabel.position = CGPoint(x: 980, y: 650)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if objects.contains(editLabel) {
            ...
            if editingMode {
                ...
                ballsLimit -= 1
                location.y = 768
                let colors = ["Green", "Red", "Cyan", "Grey", "Blue", "Purple", "Yellow"]
    ...
    func collision(between ball: SKNode, object: SKNode) {
        if object.name == "good" {
            guard let isGreen = ball.name?.contains("Green") else { return }
            if isGreen {
                ballsLimit += 1
            }
            destroy(ball: ball)
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        guard let nodeAisBall = nodeA.name?.contains("ball") else { return }
        guard let nodeBisBall = nodeB.name?.contains("ball") else { return }
